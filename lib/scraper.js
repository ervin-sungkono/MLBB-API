...
import { wait, convertToJsonAttribute, convertToThreeDigitID } from "./helper";
...
export async function fetchListHeroes(){
    ...
    await put('output/hero.json', jsonData, {access: 'public', addRandomSuffix: false});
...
export async function fetchListEquipment(){
    ...
    const jsonData = JSON.stringify(uniqueContents.map((d, idx) => ({
        id: `EQ${convertToThreeDigitID(idx)}`,
        ...d
    })), null, 2)

    // Upload JSON to Vercel Blob
    await put('output/equipment.json', jsonData, {access: 'public', addRandomSuffix: false});
...
export async function fetchHeroDetails(offset = 0, limit = 40){
    ...
    await Promise.all(heroDetails.map(async(heroDetail) => (
        await put(`hero/${heroDetail.id}.json`, JSON.stringify(heroDetail, null, 2), { access: 'public', addRandomSuffix: false })
    )))
...
export async function fetchEquipmentDetails(offset = 0, limit = 40){
    ...
    await cluster.idle()
    console.log("equipments:", equipmentDetails)
    // Upload to blob storage for each JSON object
