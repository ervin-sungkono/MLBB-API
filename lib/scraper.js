...
import { convertToJsonAttribute, convertToThreeDigitID, getDateDifference } from "./helper";
    ...
    try{
        ...
        const performTask = async({hero, url}, index) => {
            try{
                ...
                if(heroDetail.lastUpdated && getDateDifference(new Date(heroDetail.lastUpdated)) < 7){
            ...
            try{
                ...
                await page.waitForSelector('[data-index-number="2"] tbody tr')
    ...
    try{
        ...
        const performTask = async({equipment, url}, index) => {
            try{
                ...
                if(equipmentDetail.lastUpdated && getDateDifference(new Date(equipmentDetail.lastUpdated)) < 7){
                    console.log(`Skipped navigating to ${url}`)
                    return
                }
            }catch(err){}
