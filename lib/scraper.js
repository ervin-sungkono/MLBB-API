...
export async function fetchListHeroes(){
    ...
    try{
        ...
        return await put('output/hero.json', jsonData, {access: 'public', addRandomSuffix: false, cacheControlMaxAge: 0});
    }catch(err){
        throw new Error(err)
    }finally{
...
export async function fetchListEquipment(){
    ...
    try{
        ...
        // Upload JSON to Vercel Blob
        return await put('output/equipment.json', jsonData, {access: 'public', addRandomSuffix: false, cacheControlMaxAge: 0});
    }catch(err){
        throw new Error(err)
    }finally{
        console.log("Closing browser")
...
export async function fetchHeroDetails(offset = 0, limit = 20){
    ...
    try{
        ...
        return await Promise.all(result.map(async(heroDetail) => (
            put(`hero/${heroDetail.id}.json`, JSON.stringify(heroDetail, null, 2), { access: 'public', addRandomSuffix: false })
        )))
    }catch(err){
        throw new Error(err)
    }finally{
...
export async function fetchEquipmentDetails(offset = 0, limit = 20){
    ...
    try{
        ...
        // Upload to blob storage for each JSON object
        return await Promise.all(result.map(async(equipmentDetail) => (
            put(`equipment/${equipmentDetail.id}.json`, JSON.stringify(equipmentDetail, null, 2), { access: 'public', addRandomSuffix: false })
        )))    
    }catch(err){
        throw new Error(err)
    }finally{
