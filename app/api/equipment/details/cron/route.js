...
export async function GET(){
    try{
        ...
        const results = await Promise.all(offsets.map(async(offset) => await fetch(`https://${process.env.VERCEL_URL}/api/equipment/details?offset=${offset}&limit=${SCRAPING_LIMIT}`)))
        console.log(results)
